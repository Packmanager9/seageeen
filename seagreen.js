...
window.addEventListener('DOMContentLoaded', (event) => {
    ...
    let pomaoimg = new Audio()
    pomaoimg.src = 'seagreen.mp3'
    ...
    class Circle {
        ...
        frictiveMove() {
            if (this.reflect == 1) {
                ...
                if (this.y + this.radius > canvas.height-120) {
                    if (this.ymom > 0) {
                        this.ymom -= .11
                    }
                }
    ...
    function setUp(canvas_pass, style = "#000000") {
        ...
        window.addEventListener('pointerdown', e => {
            ...
            //     while (waves[t].link.hypotenuse() < 100) {
            //         // waves[t].wiggle = Math.max(-waves[t].link.hypotenuse(), -100)
            //         waves[t].wiggle++
            //         waves[t].y++
            //     }
            // }
    ...
    let worldangle = 0
    let step = 0
    let ball = new Circle(640, 360, 12, "white", 0, 0, .97)
    ball.sparks = []
    ball.wet = 0 
    ball.reflect = 1
    ball.score = 0
    let waves = []
    for (let t = 0; t < 480; t++) {
        let rect = new Rectangle(t * 3, 360, 3, 1000, "#00AAFF88")
        rect.link = new LineOP(rect, ball)
        rect.wiggle = 0
        rect.antiwiggle = 0
        rect.dir = 0
        rect.residue = 0
    ...
    class Score {
        constructor(x) {
            this.y = 0
            this.x = 10 + (Math.random() * 1260)
            this.radius = 14
            this.value = 3
            this.time = 100
            this.ymom = (Math.random()) + 1
            this.xmom = 0
            this.color = "#00FF00"
            this.link = new LineOP(ball, this)
        }
        draw() {
            this.time--
            if (!(this.marked > -1)) {
                ...
                canvas_context.fillStyle = this.color
                canvas_context.fillText("+" + this.value, this.x - 10, this.y)
            }
        }
        move() {
            if (!(this.marked > -1)) {
    ...
    function main() {
        ...
        //     recording = 0
        //     video_recorder.stop()
        //     video_recorder.download('seagreen.webm')
        // }
        ...
        for (let t = 0; t < waves.length; t++) {
            let j = 0
            while (waves[t].link.hypotenuse() <= ball.radius || (Math.abs(waves[t].x - ball.x) < ball.radius && waves[t].y < ball.y)) {
                waves[t].y += 1
                ball.wet = 1
                if(ball.ymom > -6) {
                ball.ymom -= .0022 * (waves[t].link.hypotenuse())
                }
                if (ball.ymom < 0 && ball.ymom > -6) {
                    ball.ymom *= 1.02
                }
                if (ball.xmom > 0) {
                    if (keysPressed['a']) {
                        if (j == 0) {
                            if (waves[t].link.hypotenuse() <= ball.radius) {
                                j = 1
                                if (Math.random() < .9) {
                                    let spark = new Circle(waves[t].x, waves[t].y, 1, "#00FFFF", (ball.xmom * 2) + 3, -2 - Math.random())
                                    spark.life = 7 + (Math.random() * 10)
                                    if (Math.random() < .1) {
                                        spark.color = "#ffffff"
                                    }
                                    ball.sparks.push(spark)
                                    waves[t].y += 2
                                }
                            }
                        }
                ...
                waves[t].antiwiggle = 0
                waves[t].residue = 0
                waves[t].wiggle = waves[t].y - 360
                waves[t].wiggle*=.5
                waves[t].antiwiggle = waves[t].wiggle
        ...
        for (let t = 0; t < waves.length; t++) {
            if (t < waves.length - 1) {
                if (t > 0) {
                    waves[t].dir = Math.sign(waves[t + 1].y - waves[t - 1].y)
                }
            }
            if (t > 0 && waves[t].dir == 1) {
                waves[t - 1].wiggle = ((waves[t].wiggle * 1) + (waves[t - 1].wiggle * 1)) * .5
                waves[t - 1].antiwiggle = ((waves[t].antiwiggle * 1) + (waves[t - 1].antiwiggle * 1)) * .5
            } else if (t < waves.length - 1) {
                waves[t + 1].wiggle = ((waves[t].wiggle * 1) + (waves[t + 1].wiggle * 1)) * .5
                waves[t + 1].antiwiggle = ((waves[t].antiwiggle * 1) + (waves[t + 1].antiwiggle * 1)) * .5
            }
            waves[t].antiwiggle -= .03 * waves[t].wiggle
            waves[t].residue -= Math.abs(.03 * waves[t].wiggle)
            waves[t].wiggle *= .997
            waves[t].wiggle += .03 * waves[t].antiwiggle
            waves[t].residue += Math.abs(.03 * waves[t].antiwiggle)
            waves[t].antiwiggle *= .997
            waves[t].y = 360 + waves[t].wiggle + waves[t].antiwiggle
            waves[t].residue *= .999
            if (count2 ==  1) {
                first = 0
                count2 = 0
            ...
            for (let k = 0; k < waves.length; k++) {
                if (first <= 640*3) {
                    first += .01
                    waves[t].wiggle += Math.sin(.15 * (worldangle + t + (step))) * .0065
                }
                if (t > 0 && waves[t].dir == 1) {
                    waves[t].wiggle += waves[t - 1].residue * .001
                    waves[t - 1].residue *= .999
                } else if (t < waves.length - 1) {
                    waves[t].wiggle += waves[t + 1].residue * .001
                    waves[t + 1].residue *= .999
                }
        ...
        for (let t = 0; t < ball.sparks.length; t++) {
            if (ball.sparks[t].life < 0) {
                ball.sparks.splice(t, 1)
            }
        }
        for (let t = 0; t < ball.sparks.length; t++) {
            ball.sparks[t].move()
        ...
        if (count == 10) {
            count = 0
            let point = new Score(0)
            points.push(point)
        }

        for (let t = 0; t < points.length; t++) {
            if (points[t].marked <= 0) {
                points.splice(t, 1)
            } else {
                points[t].marked--
            }
        ...
        for (let t = 0; t < points.length; t++) {
            ...
            if (!(Math.abs(now - Date.now()) > 120000)) {
                if (points[t].link.hypotenuse() <= points[t].radius + ball.radius) {
                    if(points[t].marked > -1){

                    }else{

                    ball.score += points[t].value
                    points[t].marked = 25
                    }
                }
            }
        ...
        canvas_context.font = "20px arial"
        canvas_context.fillStyle = ball.color
        canvas_context.fillText("Score: "+ball.score, 10, 20)
        // canvas_context.fillText(count2, 10, 70 )

        canvas_context.font = "20px arial"
        canvas_context.fillStyle = "white"
        if((120 - (Math.ceil(Math.ceil(Math.abs(now - Date.now())) / 1000))) > 0){

            canvas_context.fillText("Time: "+ (120 - (Math.ceil(Math.ceil(Math.abs(now - Date.now())) / 1000))), 10, 50)
        }else{

        canvas_context.fillText("Time: "+ "Finished", 10, 50)
        }
        ball.draw()
        pomaoimg.volume = .1
        pomaoimg.play()
